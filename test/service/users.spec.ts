...
describe('UserService', () => {
  ...
  describe('회원가입 검증', () => {
    ...
    it('비밀번호 validation.', async () => {
      // 이메일이 존재하지 않음을 mock
      jest.spyOn(userRepository, 'findOneBy').mockResolvedValue(undefined);
      const passwordValidateUser = new UsersEntity();
      const test_password_validation = {
        username: 'ash',
        email: 'ash@gmail.com',
        password: 'Pass',
        phone: '21342',
      }
      Object.assign(passwordValidateUser, test_password_validation);

      // userService.signUp 호출 시 BadRequestException 예외가 발생하는 것을 검증합니다.
      await expect(userService.signUp(passwordValidateUser)).rejects.toThrow(BadRequestException);

      // 예외 메시지까지 검증하려면 다음과 같이 합니다.
      await expect(userService.signUp(passwordValidateUser)).rejects.toThrow(
